...
# Previsão de vencedor usando os modelos treinados
df_futuro['Previsao_Vencedor'] = np.where(
    df_futuro['Previsao_Gols'] > df_futuro[['FT_Goals_A']].mean(axis=1), df_futuro_with_home_away['Home'], 
    np.where(df_futuro['Previsao_Gols'] < df_futuro[['FT_Goals_A']].mean(axis=1), df_futuro_with_home_away['Away'], 'Draw')
)
