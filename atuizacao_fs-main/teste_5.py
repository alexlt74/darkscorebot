...
# Manter cópia das colunas 'Home' e 'Away'
df_historico_original = df_historico[['Home', 'Away']].copy()
...
df_historico['Winner'] = np.where(df_historico['FT_Goals_H'] > df_historico['FT_Goals_A'], df_historico_original['Home'],
                                 np.where(df_historico['FT_Goals_H'] < df_historico['FT_Goals_A'], df_historico_original['Away'], 'Draw'))
...
# Manter colunas 'Home', 'Away', 'Date' e 'Time' para impressão depois
df_futuro_with_home_away = df_futuro[['Home', 'Away', 'Date', 'Time']].copy()
...
# Adicionar previsões de escanteios e cartões (exemplo)
df_futuro['Previsao_Escanteios'] = np.random.randint(5, 15, size=len(df_futuro))  # Exemplo de previsão
df_futuro['Previsao_Cartoes'] = np.random.randint(1, 5, size=len(df_futuro))  # Exemplo de previsão

# Juntar as colunas 'Home', 'Away', 'Date', 'Time' com as previsões
...
df_futuro_with_home_away['Previsao_Escanteios'] = df_futuro['Previsao_Escanteios']
df_futuro_with_home_away['Previsao_Cartoes'] = df_futuro['Previsao_Cartoes']
